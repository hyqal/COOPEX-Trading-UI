@MarketData = flight.component ->
  ...
  @reqK = (market, minutes, limit = 768) ->
    ...
    url = "http://ex.cexo.io/api/v2/k_with_pending_trades.json?market=#{market}&limit=#{limit}&period=#{minutes}&trade_id=#{tid}"
    order_book_url = "http://ex.cexo.io/api/v2/depth.json?market=#{market}"
    $.getJSON url, (data) =>
